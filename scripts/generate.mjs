...
assign('diffs-dark-bg',v('canvas')); assign('diffs-dark',v('text')); assign('diffs-dark-addition-color',v('success')); assign('diffs-dark-deletion-color',v('danger'));
const props = obj => Object.entries(obj).map(([k,value])=>`  --${k}: ${value};`).join('\n');
const tokens = `/* Generated from palette.json. SiteKit provenance: docs/research.md. */\n:root.dark {\n${props(Object.fromEntries(Object.entries(p).map(([k,value])=>[`kujo-${k}`,value])))}\n  --kujo-label-font: "Kujo Departure Mono", var(--font-mono);\n${props(map)}\n  color-scheme: dark;\n}\n`;
...
const css = `/* Generated by scripts/generate.mjs. Edit palette.json, surfaces.css, signal.css. */\n@font-face { font-family: "Kujo Departure Mono"; src: url(data:font/woff2;base64,${font}) format("woff2"); font-display: swap; font-weight: 400; }\n${tokens}\n:root.dark { --kujo-workflow: url(data:image/webp;base64,${workflow}); --kujo-mark: url(data:image/svg+xml;base64,${mark}); }\n${await read('themes/surfaces.css')}\n${await read('themes/signal.css')}`;
...
const code = {name:'Kujo',type:'dark',colors,tokenColors:rules.map(([name,scope,key])=>({name,scope,settings:{foreground:p[key]}}))};
for(const [file,content] of [['themes/tokens.css',tokens],['themes/kujo.css',css],['themes/kujo-code.json',JSON.stringify(code,null,2)+'\n']]) {
