...
try {
 ...
 for(const [phase,time] of [['still',0],['signal',8830]]) {
  ...
  await page.screenshot({path:`screenshots/background-${phase}.png`});
 }
 ...
 await page.screenshot({path:'screenshots/background-reduced-motion.png'});
 await mkdir('artifacts',{recursive:true});
 await writeFile('artifacts/signature-qa.json',JSON.stringify({date:new Date().toISOString(),states,reducedMotion:true,brand:await page.locator('[data-sidebar=sidebar] > .shrink-0').first().evaluate(e=>getComputedStyle(e,'::before').content)},null,2)+'\n');
