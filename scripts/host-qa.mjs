...
try {
 ...
 await page.getByRole('button',{name:'Light mode',exact:true}).click();await shot('light-fallback');
 ...
 command('theme','set','default');await page.waitForTimeout(1500);assert.equal(await page.locator('.bb-kujo-signal').evaluate(e=>getComputedStyle(e).display),'none');
 command('theme','set','plugin:bb-kujo:kujo');await page.waitForTimeout(1500);
 command('plugin','disable','bb-kujo');await page.waitForTimeout(1500);assert.equal(await page.locator('.bb-kujo-signal').count(),0);
 ...
 await writeFile('artifacts/host-qa.json',JSON.stringify({date:new Date().toISOString(),url,browser:browser.version(),errors,reloads:5,unload:true,themeSwitch:true,reducedMotion:true},null,2)+'\n');
